...
TaskWatchWebSocket::TaskWatchWebSocket(QObject *parent)
...
void TaskWatchWebSocket::watchWorkspace(const QString &workspaceId)
{
    ...
    const QString path =
        QStringLiteral("/api/v2/workspaces/%1/watch-ws").arg(workspaceId);
...
void TaskWatchWebSocket::onTextMessage(const QString &message)
{
    ...
    const QJsonObject latestBuild =
        workspace.value(QLatin1String("latest_build")).toObject();
    const QString buildStatus =
        latestBuild.value(QLatin1String("status")).toString();
    stateEntry.message = buildStatus;
    const QString ts =
        latestBuild.value(QLatin1String("updated_at")).toString();
    if (!ts.isEmpty())
        stateEntry.timestamp = QDateTime::fromString(ts, Qt::ISODate);
...
TaskStatus TaskWatchWebSocket::deriveTaskStatus(const QJsonObject &workspace) const
{
    ...
    const QString transition =
        latestBuild.value(QLatin1String("transition")).toString();

    // Map workspace build status + transition to a TaskStatus.
    if (buildStatus == QLatin1String("pending"))
        return TaskStatus::Pending;
    if (buildStatus == QLatin1String("starting"))
        return TaskStatus::Initializing;
    if (buildStatus == QLatin1String("running")) {
        if (transition == QLatin1String("stop")
            || transition == QLatin1String("delete"))
            return TaskStatus::Paused;
        return TaskStatus::Active;
    }
    if (buildStatus == QLatin1String("stopping")
        || buildStatus == QLatin1String("canceling")
        || buildStatus == QLatin1String("deleting"))
        return TaskStatus::Paused;
    if (buildStatus == QLatin1String("stopped")
        || buildStatus == QLatin1String("canceled")
        || buildStatus == QLatin1String("deleted"))
        return TaskStatus::Paused;
    if (buildStatus == QLatin1String("failed"))
