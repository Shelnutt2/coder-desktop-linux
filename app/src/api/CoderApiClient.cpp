...
void CoderApiClient::setSessionToken(const QString& token) {
    ...
    if (wasAuth != isAuthenticated()) emit authStateChanged();
}
...
QNetworkReply* CoderApiClient::createWorkspaceBuild(const QString& workspaceId,
    ...
    return post(QStringLiteral("/api/v2/workspaces/%1/builds").arg(workspaceId), body);
}
...
void CoderApiClient::fetchWorkspaces(const QString& query) {
    ...
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        ...
        const QJsonArray arr = doc.object().value(QLatin1String("workspaces")).toArray();
...
QNetworkReply* CoderApiClient::post(const QString& path, const QJsonObject& body) {
    QNetworkReply* reply =
...
QNetworkReply* CoderApiClient::put(const QString& path, const QJsonObject& body) {
    QNetworkReply* reply =
        m_nam->put(buildRequest(path), QJsonDocument(body).toJson(QJsonDocument::Compact));
    connectErrorHandler(reply, path);
    return reply;
}
...
void CoderApiClient::connectErrorHandler(QNetworkReply* reply, const QString& endpoint) {
    connect(reply, &QNetworkReply::finished, this, [this, reply, endpoint]() {
        const int status = reply->attribute(QNetworkRequest::HttpStatusCodeAttribute).toInt();
        if (reply->error() != QNetworkReply::NoError) {
            const QString msg = reply->errorString();
            qWarning() << "CoderApiClient: request failed:" << endpoint << "status:" << status
                       << msg;
