...
public:
    ...
    static TaskStateEntry fromJson(const QJsonObject& obj) {
        ...
        if (!ts.isEmpty()) e.timestamp = QDateTime::fromString(ts, Qt::ISODate);
        e.state = taskStateFromString(obj.value(QLatin1String("state")).toString());
...
public:
    ...
    static Task fromJson(const QJsonObject& obj) {
        ...
        t.status = taskStatusFromString(obj.value(QLatin1String("status")).toString());

        const QJsonObject stateObj = obj.value(QLatin1String("current_state")).toObject();
        if (!stateObj.isEmpty()) t.currentState = TaskStateEntry::fromJson(stateObj);

        const QString created = obj.value(QLatin1String("created_at")).toString();
        if (!created.isEmpty()) t.createdAt = QDateTime::fromString(created, Qt::ISODate);

        const QString updated = obj.value(QLatin1String("updated_at")).toString();
        if (!updated.isEmpty()) t.updatedAt = QDateTime::fromString(updated, Qt::ISODate);
    ...
    static TaskStatus statusFromString(const QString& s) { return taskStatusFromString(s); }

    static TaskState stateFromString(const QString& s) { return taskStateFromString(s); }

    static QString statusToString(TaskStatus s) { return taskStatusToString(s); }

    static QString stateToString(TaskState s) { return taskStateToString(s); }
};
