...
public:
    ...
    static Agent fromJson(const QJsonObject &obj)
    {
        ...
        a.status = agentStatusFromString(
            obj.value(QLatin1String("status")).toString());
        a.apps   = WorkspaceApp::listFromJson(
            obj.value(QLatin1String("apps")).toArray());
        return a;
    }
...
public:
    ...
    static Workspace fromJson(const QJsonObject &obj)
    {
        ...
        w.health       = obj.value(QLatin1String("health")).toObject()
                             .value(QLatin1String("healthy")).toBool()
        ...
        const QJsonObject latestBuild =
            obj.value(QLatin1String("latest_build")).toObject();
        w.latestBuildId = latestBuild.value(QLatin1String("id")).toString();
        w.status = workspaceStatusFromString(
        ...
        const QJsonArray resources =
            latestBuild.value(QLatin1String("resources")).toArray();
        for (const QJsonValue &rv : resources) {
            const QJsonArray agentsArr =
                rv.toObject().value(QLatin1String("agents")).toArray();
            for (const QJsonValue &av : agentsArr)
                w.agents.append(Agent::fromJson(av.toObject()));
        }
        return w;
    }
