...
void TaskModel::setTasks(const QList<TaskInfo>& tasks) {
    ...
    for (const auto& t : tasks) newIds.insert(t.id);
    ...
    for (int i = 0; i < m_tasks.size(); ++i) currentPositions[m_tasks[i].id] = i;
    ...
    qDebug() << "[TaskModel] setTasks: input=" << tasks.size() << "removed=" << removed
             << "updated=" << updated << "added=" << added << "total=" << m_tasks.size();
...
bool TaskModel::isLoading() const {
    return m_loading;
}
...
QString TaskModel::errorMessage() const {
