...
static void handle_security_context_commit(struct wl_listener *listener,
                                            void *data) {
    struct coder_dlp_compositor *comp =
        wl_container_of(listener, comp, security_context_commit);
    ...
    fprintf(stderr, "coder-dlp: security context commit: app_id=%s engine=%s\n",
            ...
            event->state->sandbox_engine ? event->state->sandbox_engine
                                         : "(none)");
}
...
void dlp_security_context_init(struct coder_dlp_compositor *comp) {
    ...
    comp->security_context_mgr =
        wlr_security_context_manager_v1_create(comp->wl_display);
    if (!comp->security_context_mgr) {
        fprintf(stderr,
                "coder-dlp: failed to create security context manager\n");
        return;
    }

    comp->security_context_commit.notify = handle_security_context_commit;
    wl_signal_add(&comp->security_context_mgr->events.commit,
                  &comp->security_context_commit);
}
