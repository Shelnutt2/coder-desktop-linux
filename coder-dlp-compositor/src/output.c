...
static void handle_output_frame(struct wl_listener *listener, void *data) {
    ...
    struct coder_dlp_compositor *comp =
        wl_container_of(listener, comp, output_frame);

    struct wlr_scene_output *scene_output =
        wlr_scene_get_scene_output(comp->scene, comp->output);
    if (!scene_output) {
        return;
    }
...
static void handle_output_destroy(struct wl_listener *listener, void *data) {
    (void)data;
    struct coder_dlp_compositor *comp =
        wl_container_of(listener, comp, output_destroy);
...
void compositor_handle_new_output(struct wl_listener *listener, void *data) {
    struct coder_dlp_compositor *comp =
        wl_container_of(listener, comp, new_output);
    ...
    wl_signal_add(&output->events.request_state,
                  &comp->output_request_state);
